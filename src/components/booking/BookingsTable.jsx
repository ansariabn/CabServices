	...
	const filterBooknigs = (startDate, endDate) => {
		...
		if (startDate && endDate) {
			filtered = bookingInfo.filter((booking) => {
				const bookingStarDate = parseISO(booking.travelInDate)
				const bookingEndDate = parseISO(booking.travelOutDate)
				return (
			...
			<table className="table table-bordered table-hover shadow">
				<thead>
					<tr>
						...
						<th>Cab ID</th>
						<th>Cab Type</th>
						<th>Travel-In Date</th>
						<th>Travel-Out Date</th>
						<th>Guest Name</th>
						<th>Guest Email</th>
						<th>Adults</th>
						<th>Children</th>
				...
				<tbody className="text-center">
					{filteredBookings.map((booking, index) => (
						<tr key={booking.id}>
							<td>{index + 1}</td>
							<td>{booking.id}</td>
							<td>{booking.cab.id}</td>
							<td>{booking.cab.cabType}</td>
							<td>{booking.travelInDate}</td>
							<td>{booking.travelOutDate}</td>
